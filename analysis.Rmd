...
```{r}
batting2 = filter(batting, batting$yearID > 1985) %>% select(yearID,H,X2B,X3B,HR,AB,BB,HBP,SF,) %>% na.omit() %>% mutate(TB = H+2*X2B+(3*X3B)+(4*HR)) %>% mutate(SLG = TB/AB) %>% na.omit() %>% mutate(OBP = (H + BB + HBP)/(AB + BB + SF + HBP)) %>% mutate(OPS = SLG + OBP) %>% mutate(steroidEra = !(yearID>2003))
...
```{r}
pitching2 = filter(pitching, pitching$yearID > 1985) %>% select(yearID,ERA) %>% na.omit %>% mutate(steroidEra = !(yearID>2003))
